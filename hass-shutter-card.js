class ShutterCard extends HTMLElement {
  set hass(hass) {
    ...
    if (!this.card) {
      ...
      entities.forEach(function (entity) {
        ...
        shutter.innerHTML =
          ...
          (titlePosition == "bottom" ? 'style="display:none;"' : "") +
          `>
          ` +
          (buttonsPosition == "left" ? '<div style="flex: 5"></div>' : "") +
          `
              <div class="sc-shutter-position">
              
              </div>
              ` +
          (buttonsPosition == "right" ? '<div style="flex: 5"></div>' : "") +
          `
            </div>
            <div class="sc-shutter-middle" style="flex-direction: ` +
          (buttonsPosition == "right" ? "row-reverse" : "row") +
          `;">
        ...
        let mouseUp = function (event) {
          ...
          if (newPosition < _this.minPosition) newPosition = _this.minPosition;

          if (newPosition > _this.maxPosition) newPosition = _this.maxPosition;

          let percentagePosition =
            ((newPosition - _this.minPosition) * 100) /
            (_this.maxPosition - _this.minPosition);

          if (invertPercentage) {
            _this.updateShutterPosition(hass, entityId, percentagePosition);
          } else {
            _this.updateShutterPosition(
              hass,
              entityId,
        ...
        shutter
          .querySelectorAll(".sc-shutter-button")
      ...
      allShutters.appendChild(clear);

      const style = document.createElement("style");
      style.textContent = `
          .clear { clear:both }
          .sc-shutters { padding: 16px; display: flex; justify-content: center; }
            .sc-shutter { overflow: hidden; float:left }
            .sc-shutter:first-child { margin-top: 0; }
            .sc-shutter-middle { display: flex; width: 160px; margin: auto; }
              .sc-shutter-buttons { flex: 1; text-align: center; margin-top: 0.4rem; }
              .sc-shutter-selector { flex: 1; }
                .sc-shutter-selector-picture { position: relative; margin: auto; background-size: cover; min-height: 109px; max-height: 100%; width: 111px; }
                ...
                .sc-shutter-selector-slide { position: absolute; top: 12px; left: 6.5px; width: 88%; height: 0; }
                ...
                .sc-shutter-selector-picker { position: absolute; top: 12px; left: 6.5px; width: 88%; cursor: pointer; height: 20px; background-repeat: no-repeat; }
            ...
            .sc-shutter-top { display: flex; flex-direction: row; text-align: center; margin-bottom: 1rem; }
            .sc-shutter-bottom { text-align: center; margin-top: 1rem; }
              .sc-shutter-position { display: block; flex:11; vertical-align: middle; padding: 0 6px; border-radius: 2px;}
        `;
    ...
    entities.forEach(function (entity) {
      ...
      const shutter = _this.card.querySelector(
        'div[data-shutter="' + entityId + '"]'
      );
      ...
      if (!_this.isUpdating) {
        shutter
          .querySelectorAll(".sc-shutter-position")
          .forEach(function (shutterPosition) {
  ...
  setPickerPositionPercentage(position, picker, slide) {
    let realPosition =
      ((this.maxPosition - this.minPosition) * position) / 100 +
      this.minPosition;
